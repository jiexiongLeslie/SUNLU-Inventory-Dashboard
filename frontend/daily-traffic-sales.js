(function() {
  ...
  var storeBody = document.getElementById('storeBody');
  ...
  function renderKpis() {
    ...
    var items = [
      ['总销售额', formatMoney(totals.total_sales), 'total_sales'],
      ['订单数', formatNumber(totals.orders), 'orders'],
      ['访客数', formatNumber(totals.online_store_visitors), 'online_store_visitors'],
      ['Sessions', formatNumber(totals.sessions), 'sessions'],
      ['订单转化率', formatPercent(totals.order_conversion_rate), 'orders / sessions'],
      ['客单价', formatMoney(totals.sales_per_order), 'total_sales / orders'],
  ...
  function renderCharts() {
    ...
    charts.daily = new Chart(document.getElementById('dailyChart'), {
      ...
      data: {
        ...
        datasets: [
          { label: '销售额', data: daily.map(function(item) { return item.total_sales; }), borderColor: '#2f80ed', backgroundColor: 'rgba(47,128,237,.12)', tension: .28, yAxisID: 'money' },
          { label: '订单', data: daily.map(function(item) { return item.orders; }), borderColor: '#218a54', backgroundColor: 'rgba(33,138,84,.12)', tension: .28, yAxisID: 'count' },
          { label: '访客', data: daily.map(function(item) { return item.online_store_visitors; }), borderColor: '#b7791f', backgroundColor: 'rgba(183,121,31,.10)', tension: .28, yAxisID: 'count' },
          { label: 'Sessions', data: daily.map(function(item) { return item.sessions; }), borderColor: '#805ad5', backgroundColor: 'rgba(128,90,213,.10)', tension: .28, yAxisID: 'count' }
        ]
      },
      options: {
        responsive: true,
        ...
        scales: {
          money: { beginAtZero: true, ticks: { color: '#6b7280' }, grid: { color: '#edf2f7' } },
          count: { beginAtZero: true, position: 'right', grid: { drawOnChartArea: false }, ticks: { color: '#6b7280' } },
  ...
  function renderStoreSummary() {
    var stores = state.data?.breakdowns?.by_store || [];
    storeCount.textContent = formatNumber(stores.length) + ' 个站点';
    if (!stores.length) {
      storeBody.innerHTML = '<tr><td colspan="7"><div class="empty">暂无数据</div></td></tr>';
      return;
    }
    storeBody.innerHTML = stores.map(function(row) {
      return '<tr>' +
        '<td><span class="badge">' + escapeHtml(row.label || '-') + '</span></td>' +
        '<td class="number">' + formatMoney(row.total_sales) + '</td>' +
        '<td class="number">' + formatNumber(row.orders) + '</td>' +
        '<td class="number">' + formatNumber(row.online_store_visitors) + '</td>' +
        '<td class="number">' + formatNumber(row.sessions) + '</td>' +
        '<td class="number">' + formatPercent(row.order_conversion_rate) + '</td>' +
        '<td class="number">' + formatMoney(row.sales_per_order) + '</td>' +
      '</tr>';
    }).join('');
  }
  ...
  function matchesSearch(row, q) {
    if (!q) return true;
    return [row.store_label, row.shop_name, row.day].join(' ').toLowerCase().indexOf(q) >= 0;
  }
  ...
  function renderTable() {
    ...
    if (!pageRows.length) {
      tableBody.innerHTML = '<tr><td colspan="9"><div class="empty">暂无匹配数据</div></td></tr>';
      return;
    }
  ...
  function render() {
    ...
    if (state.data) {
      ...
      meta.textContent = stores + ' · sales + sessions · ' + state.data.since + ' 至 ' + state.data.until + (cacheText ? ' · ' + cacheText : '');
    }
  }
  ...
  function loadData(refresh) {
    ...
    setLoading(true, refresh ? '正在刷新每日访问销量数据...' : '正在读取每日访问销量缓存...');
    fetchData(refresh).then(function(data) {
      state.data = data;
      state.rows = data.rows || [];
      state.filteredRows = state.rows;
      setLoading(false, '已加载 ' + formatNumber(state.rows.length) + ' 条每日访问销量数据');
      render();
    }).catch(function(err) {
      state.data = null;
      state.rows = [];
      ...
      kpiGrid.innerHTML = '';
      storeBody.innerHTML = '<tr><td colspan="7"><div class="empty">加载失败</div></td></tr>';
      renderTable();
