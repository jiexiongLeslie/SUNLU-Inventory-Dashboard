...
# HTML模板 - 使用单大括号，避免与Python format冲突
...
/* Default Data - 最新库存数据 2026-05-06 */
...
function populateCategoryFilter() {
  const cats = DATA.map(d => d.category).filter((v, i, a) => a.indexOf(v) === i).sort((a, b) => a.localeCompare(b, 'zh-CN'));
...
print(f"\n看板已生成: {output_file}")
print(f"文件大小: {len(html_template):,} 字符")
