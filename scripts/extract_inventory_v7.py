...
def extract_inventory(file_path):
    ...
    for cfg in configs:
        ...
        # 如果没找到品类列，使用默认值
        if cat_col == -1:
            cat_col = 0 if cfg['region'] != '美国' else 1
            
        print(f"  品类列={cat_col}, 颜色列={color_col}, SKU列={store_sku_col}")
        ...
        print(f"  提取 {row_count} 条记录, 总库存={total_stock:,}")
...
def main():
    ...
    print("=" * 60)
    print("SUNLU 库存数据提取 v7 - 修复美国站列位置")
    print("=" * 60)
    ...
    output_file = r'C:\Users\Administrator\Documents\trae\inventory_data_v7.json'
