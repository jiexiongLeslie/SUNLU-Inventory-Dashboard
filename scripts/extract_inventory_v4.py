...
# -*- coding: utf-8 -*-
"""
SUNLU 库存数据提取 v4 - 按列名动态查找
- 英国站：表头包含"总库存"（不含"欧洲"）
- 全球-欧洲：表头包含"总库存"且包含"欧洲所有通用"
- 店铺SKU：表头包含"店铺SKU"
"""
...
def extract_inventory(file_path):
    ...
    for cfg in configs:
        ...
        print(f"  颜色列={color_col}, SKU列={store_sku_col}, 库存列={stock_col}")
        if stock_col >= 0:
            print(f"  库存列表头: '{str(header.iloc[stock_col]).replace(chr(10), ' ')}'")
...
def main():
    ...
    print(f"合计: {len(data)} 条, 总库存 {uk_stock + eu_stock:,}")
    print("=" * 60)
    
    output_file = r'C:\Users\Administrator\Documents\trae\inventory_data_v4.json'
